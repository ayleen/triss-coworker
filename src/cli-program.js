...
// Side-effect-free construction of the Triss CLI Commander tree.
//
// buildProgram() only registers commands, options, and descriptions and
// returns the Commander program — it never parses argv, never reads env
// files, never starts the passive update check, and never loads tracker
// integrations on its own (the caller passes them in). bin/triss.js uses it
// for the real executable; scripts/generate-public-reference.js and the
// docs contract tests use it to inventory the exact same tree without any
// startup side effects. Do not add side effects to this module: keep them
// in bin/triss.js so the executable path stays the only entry that has them.
...
import { runCommitMsg } from './commands/commit-msg.js';
...
import {
  ...
  runCoderResultList,
  runCoderResultClean,
} from './commands/coder.js';
import { runCoderStateBackup, runCoderStateValidate } from './commands/coder-state-backup.js';
import { withCall } from './call-context.js';
import { positiveIntegerOption, positiveNumberOption } from './option-validation.js';
import { normalizeModelEffort } from './provider-contract.js';

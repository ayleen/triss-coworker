...
const CODER_TOOLS = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        model: { type: 'string', pattern: '^[^\\s/]+/[^\\s]+$', description: 'Override the model for this run only, as <provider>/<id> — e.g. Triss worker (triss-worker/deepseek-v4-flash), Z.AI GLM (zai-coding-plan/glm-5.2), OpenCode Zen (opencode/deepseek-v4-flash-free), OpenCode Go (opencode-go/deepseek-v4-flash), Moonshot Kimi (moonshotai/kimi-k2.7-code), or Kimi for Coding (kimi-for-coding/k3)' },
        small_model: { type: 'string', pattern: '^[^\\s/]+/[^\\s]+$', description: 'With provider, override small_model for this run; defaults to model' },

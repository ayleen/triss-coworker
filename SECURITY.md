...
path, wrong architecture) are accounted for in the VEX document:
[docs/vex/vex.json](docs/vex/vex.json) (OpenVEX format) carries one
statement per advisory with a status, a justification code, and a written
impact statement, generated by `scripts/vex-report.js` from the OSV scan
of the production dependency tree. Suppression decisions live in
[docs/vex/suppressions.json](docs/vex/suppressions.json) — an advisory
affecting an installed version without a suppression fails the
`vex:generate` gate (run on every pull request and weekly by the VEX
workflow). Dismissal without a written rationale is not accepted.

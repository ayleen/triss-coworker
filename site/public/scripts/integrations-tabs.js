...
const INTS = {
  ...
  github: { cmd: 'triss github', body: 'Search, read, create, update, and comment on issues. Pairs naturally with triss review 123 (a GitHub PR number).', env: 'GITHUB_TOKEN — or gh CLI', ops: 'search, issue, create, update, comment', lines: '$ triss github issue 812 \\\n    --question "has anyone reproduced this?"\n\n→ yes: two reports on Node 22, none on 24' },
  gitlab: { cmd: 'triss gitlab', body: 'The same five operations against a GitLab instance, self-hosted included.', env: 'GITLAB_TOKEN', ops: 'search, issue, create, update, comment', lines: '$ triss gitlab search "incident" \\\n    --question "anything still open?"\n\n→ 1 open: #204, waiting on infra' },
};
let active = 'jira';
function renderInt() {
  const d = INTS[active];

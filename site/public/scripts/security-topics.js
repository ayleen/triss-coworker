...
const TOPICS = {
  ...
  log: { title: "Metadata only, and deletable", body: "Every worker call appends one record: timestamp, model, token counts, cost, label, call id, cwd. Prompt and file content never written.", vars: ["TRISS_USAGE_LOG_CWD=0 — keep log but stop recording cwd", "TRISS_USAGE_LOG=0 — no log", "triss usage --reset — delete log"] },

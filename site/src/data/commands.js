...
export const COMMANDS = [
  { name: "ask", tier: "small", group: "delegate", body: "Answer from files, URLs, or stdin through the canonical provider runtime.", flags: ["--paths <glob…>", "--urls <url…>", "--question <text>", "--provider <id>", "--model <native-id>", "--engine <id>", "--protect-credentials", "--effort <level>"], example: "$ triss ask --provider zai --model glm-5.2 --effort high --paths src --question \"find correctness defects\"" },
  { name: "review", tier: "main", group: "delegate", body: "Code review on a branch, PR, selected files, or piped diff.", flags: ["--base <ref>", "--stdin", "--files <path…>", "--provider <id>", "--model <native-id>", "--engine <id>", "--protect-credentials", "--effort <level>"], example: "$ triss review --provider moonshot --model kimi-k3" },

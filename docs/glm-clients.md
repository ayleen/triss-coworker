...
| `TRISS_CODER_OPENCODE2_VERSION` | no | Raise-only OpenCode 2 minimum (default/immutable current floor `0.0.0-beta-19059`). Triss never pins an exact build: the current qualified version and every newer parseable version remain supported when the required CLI options are present; help description wording is not a gate. Lower or malformed configured values fall back to the floor; unsupported `next/dev/tui-v2` values fail closed — see [opencode2.md](engines/opencode2.md). |

...
| `triss coder`; model-backed command with `TRISS_DEFAULT_ENGINE=opencode` | Selected local engine, then model provider | Task, selected context, tool results, engine protocol events, and either the selected raw credential or a proxy token | On explicit engine selection or persisted `TRISS_DEFAULT_ENGINE` | Keep `TRISS_DEFAULT_ENGINE=direct`; or use verified OpenCode projection. Triss injects and verifies a primary agent with `edit` and `bash` denied before launch. `--protect-credentials` keeps eligible keys behind the parent-owned proxy; otherwise warnings report raw same-UID exposure. Other non-direct engines are rejected for model projection. |

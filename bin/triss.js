...
coder
  ...
  .option('--no-isolate', 'disable worktree isolation (both engines default to isolate-OFF)')
  .option('--restrict', 'crush only: enforce the permissions.run policy (--restrict-run). Default ON for crush')
  .option('--no-restrict', 'crush only: disable the permissions.run policy (crush auto-approves every tool)')

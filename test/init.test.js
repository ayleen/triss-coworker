...
// ── INIT-08 / INIT-CODEX-05: nano block teaches the compact workflow rule ────

test('INIT-08: nano CLAUDE.md teaches the compact one-coder workflow rule', async () => {
  const origApiKey = process.env.TRISS_WORKER_API_KEY;
  const origWorkerKey = process.env.WORKER_API_KEY;
  delete process.env.TRISS_WORKER_API_KEY;
  delete process.env.WORKER_API_KEY;
  ...
  await withTmpEnv(async ({ projectDir }) => {
    ...
    const content = readFileSync(destPath, 'utf8');
    assert.ok(content.includes('Workflow:'), 'nano block should contain the compact workflow rule');
    assert.ok(content.includes('task packet'), 'nano should mention the complete task packet');
    assert.ok(
      content.includes('repository investigation, implementation, tests, debugging, and'),
    ...
    assert.ok(content.includes('fresh run without intentional session reuse'), 'nano should prefer fresh runs without session reuse');
    assert.ok(content.includes('final decision'), 'nano should reserve final decisions to the host');
    assert.ok(
      content.includes('triss agent-help'),
      'nano must keep pointing at the full reference for the complete packet and examples',
    );
  });
...
test('INIT-CODEX-05: nano AGENTS.md teaches the compact one-coder workflow rule', async () => {
  const origApiKey = process.env.TRISS_WORKER_API_KEY;
  const origWorkerKey = process.env.WORKER_API_KEY;
  delete process.env.TRISS_WORKER_API_KEY;
  delete process.env.WORKER_API_KEY;

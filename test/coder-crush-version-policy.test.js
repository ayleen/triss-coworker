...
// ─── 4. production regression matrix THROUGH runCoderRun ─────────────────────

// Isolated HOME/project root (no .env leakage, real .triss state inside tmp).
function makeIsolatedRoot() {
  return realpathSync(mkdtempSync(join(tmpdir(), 'triss-crush-policy-')));
}
...
function withIsolatedRun(repoRoot, fn) {
  return async () => {
    ...
    const origCrushMin = process.env.TRISS_CODER_CRUSH_VERSION;
    const origOcMin = process.env.TRISS_CODER_OPENCODE_VERSION;
    ...
    try {
      ...
      if (origCrushMin === undefined) delete process.env.TRISS_CODER_CRUSH_VERSION;
      else process.env.TRISS_CODER_CRUSH_VERSION = origCrushMin;
      if (origOcMin === undefined) delete process.env.TRISS_CODER_OPENCODE_VERSION;
      else process.env.TRISS_CODER_OPENCODE_VERSION = origOcMin;
      rmSync(repoRoot, { recursive: true, force: true });
    }
  };
}

// Fake spawnSync whose ONLY job is the crush --version probe answer; every
// other invocation (e.g. git worktree setup, npm) is recorded and refused.
function probeSh(installedStdout) {
  const others = [];
  const sh = (cmd, argv) => {
    if (cmd === 'crush' && argv[0] === '--version') {
      return installedStdout == null
        ? { status: 1, stdout: '', stderr: '', error: null }
        : { status: 0, stdout: installedStdout, stderr: '', error: null };
    }
    others.push([cmd, [...argv]]);
    return { status: 1, stdout: '', stderr: '', error: null };
  };
  sh.others = others;
  return sh;
...
// Side-effect recorders proving an incompatible version reaches NOTHING.
function effectRecorders() {
  const effects = { spawned: [], proxiesStarted: 0, reservations: 0 };
  return {
    effects,
    spawnFn: (cmd, argv) => {
      effects.spawned.push([cmd, ...argv]);
    ...
    startCredentialProxy: async () => {
      effects.proxiesStarted += 1;
      return { token: 't', baseUrl: 'http://127.0.0.1:0', scopedBaseUrl: 'http://127.0.0.1:0/v1', revoke() {}, closed: Promise.resolve() };
    },
    reserveSessionRow: async () => {
      effects.reservations += 1;
      throw new Error('session reservation must never be reached by an incompatible crush');
    },
...
async function assertRejected({ installed, configured }, matchers, expectedCode = null) {
  ...
  const rec = effectRecorders();
  let err = null;
  try {
    await runCoderRun('do the thing', { engine: 'crush', isolate: true, timeout: 30 }, {
      spawn: rec.spawnFn,
      spawnSync: sh,
      stdoutWrite: () => true,
      killProcess: () => true,
      processGroupPollMs: 1,
      startCredentialProxy: rec.startCredentialProxy,
      reserveSessionRow: rec.reserveSessionRow,
      releaseSessionRow: rec.releaseSessionRow,
    });
  } catch (caught) {
    err = caught;
  }
  ...
  // spawnSync saw no git/npm/worktree command beyond the version probe).
  assert.deepEqual(rec.effects.spawned, [], 'spawnCrush must be unreachable');
  assert.equal(rec.effects.proxiesStarted, 0, 'credential proxy must never start');
  assert.equal(rec.effects.reservations, 0, 'session row must never be reserved');
  assert.deepEqual(sh.others, [], 'no isolation/git side effect may run before the policy gate');
}

test(
  'matrix 1: installed 0.1.4 + configured 0.1.4 => rejected before ANY side effect',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
      { installed: 'crush version v0.1.4\n', configured: '0.1.4' },
      [/crush 0\.1\.4 found/, /minimum supported version is 0\.1\.6/],
    );
  }),
);

test(
  'matrix 2: installed 0.1.5 + no override => rejected (immutable floor)',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
...
test(
  'matrix 2b: installed 0.1.5 + configured 0.1.4 => still rejected (raise-only clamp)',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
      { installed: 'crush version v0.1.5\n', configured: '0.1.4' },
      [/minimum supported version is 0\.1\.6/],
    );
  }),
);

test(
  'matrix 3: installed 0.1.6 + malformed override => TYPED reject, no side effects',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
...
test(
  'matrix 3 (typed): the rejection carries TRISS_CODER_CRUSH_MINIMUM_INVALID',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    process.env.TRISS_CODER_CRUSH_VERSION = 'HEAD';
    const sh = probeSh('crush version v0.1.6\n');
    const rec = effectRecorders();
    await assert.rejects(
      () => runCoderRun('do the thing', { engine: 'crush', isolate: true, timeout: 30 }, {
        spawn: rec.spawnFn,
        spawnSync: sh,
        stdoutWrite: () => true,
        killProcess: () => true,
      ...
      (err) => {
        assert.equal(err.code, CRUSH_INVALID_MINIMUM_CODE);
        assert.deepEqual(rec.effects.spawned, []);
        assert.equal(rec.effects.proxiesStarted, 0);
        assert.equal(rec.effects.reservations, 0);
        assert.deepEqual(sh.others, []);
        return true;
      },
    );
  }),
...
test(
  'matrix 4: installed 0.1.6 + configured 0.2.0 => rejected against the raised minimum',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
      { installed: 'crush version v0.1.6\n', configured: '0.2.0' },
...
// Positive paths: a COMPATIBLE binary runs to completion (mirror of the
// established full-crush-flow test shape: real loopback credential proxy +
// real session rows inside the isolated root, fake engine child). The
// killProcess stub follows the existing alive/ESRCH pattern so the post-close
// residual-group check observes a dead group.
function successfulCrushChild(sessionId) {
  const child = new EventEmitter();
  child.pid = 888001;
  child.stdout = new PassThrough();
  child.stderr = new PassThrough();
...
async function runPositiveCase({ installed, configured }) {
  const sh = probeSh(installed);
  if (configured === undefined) delete process.env.TRISS_CODER_CRUSH_VERSION;
  else process.env.TRISS_CODER_CRUSH_VERSION = configured;
  let spawned = 0;
  let captured = '';
  let engineArgv = null;
  const { child, killProcess } = successfulCrushChild('crush-matrix-ok');
  await runCoderRun('do the thing', { engine: 'crush', isolate: false, timeout: 30 }, {
    spawn: (cmd, argv) => {
      spawned += 1;
      engineArgv = [cmd, ...argv];
      return child;
  ...
  return { spawned, captured, sh, engineArgv };
}

test(
  'matrix 5: installed 0.1.6 + configured 0.1.4 (below floor) => SUCCESS with effective 0.1.6',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    const { spawned, captured, engineArgv } = await runPositiveCase({
      installed: 'crush version v0.1.6\n',
      configured: '0.1.4',
    });
    assert.equal(spawned, 1);
    assert.equal(engineArgv[0], 'crush');
    assert.equal(engineArgv[1], 'run');
...
test(
  'matrix 6: installed 0.2.0 + configured 0.2.0 => SUCCESS (raised minimum honored)',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    const { spawned, captured } = await runPositiveCase({
      installed: 'crush version v0.2.0\n',
      configured: '0.2.0',
    });
    assert.equal(spawned, 1);
    const envelope = JSON.parse(captured.trim());
...
test(
  'matrix 6b: installed 0.2.0 satisfies a raised 0.2.0 but 0.1.6 does not (through runCoderRun)',
  withIsolatedRun(makeIsolatedRoot(), async () => {
    await assertRejected(
      { installed: 'crush version v0.1.6\n', configured: '0.2.0' },
      [/minimum supported version is 0\.2\.0/],
    );
  }),
);

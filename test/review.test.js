...
test('REV-11: empty model response rejects with TRISS_EMPTY_MODEL_RESPONSE and writes no answer', async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        ...
        error?.code === 'TRISS_EMPTY_MODEL_RESPONSE' &&
  ...
  assert.equal(captured.join(''), '', 'stdout must not contain a verdict');
});

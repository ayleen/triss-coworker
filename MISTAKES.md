...
- **What happened:** Owner review of the repositioned website found the
  ...
  described the standard wizard inaccurately (claimed free provider choice and
  optional host wiring; the real standard mode configures the
  `openai-compatible` profile with main+small models and wires both hosts with
  no skip).
- **Root cause:** The workflow/cwd defects were inherited from the approved
  ...
  actual CLI, and the wizard description was written from its help text
  instead of reading `src/commands/config.js`.
- **Prevention:** Documentation that prescribes a multi-step shell sequence or
  ...
  approved is not evidence that its commands work. Fix plan documents together
  with the pages that copied the defect.
